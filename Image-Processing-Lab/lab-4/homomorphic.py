...
def homomorphic_filter(image, c,Lh,Ll,D0):
    ...
    for u in range(cX):
        for v in range(cY):
            ...
            #H[u][v][1] = (Lh-Ll)*(1-np.exp(-c*(Duv**2))**2/(D0**2))+Ll
    ...
    #fshift_mask_mag = 2000 * np.log(cv2.magnitude(H[:, :, 0], H[:, :, 1]))
...
plt.title("Fig 4.    :Homomorphic input image ")
...
#magnitude_spectrum = 2000* np.log(cv2.magnitude(output_f[:,:,0], output_f[:,:,1])+1)
#plt.imshow(magnitude_spectrum),plt.title("mask of input image")
#plt.show()
...
img_back = np.log(cv2.magnitude(img_back[:, :, 0], img_back[:, :, 1]))
img_back = np.expm1(img_back)

#img_back = rescale_intensity(img_back,in_range=(0, 255))


plt.title("Fig 4.    : Homomorphic output image")
